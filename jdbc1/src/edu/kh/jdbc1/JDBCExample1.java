...
import java.sql.Statement; //beansXXX sql인거 확인하고 import해주기!

public class JDBCExample1 {
	
	public static void main(String[] args) {
		
		// JDBC (Java DataBase Connectivity) : Java에서 DB에 연결할 수 있게 해주는 Java Programming API (Java에 기존 내장된 인터페이스)
		// java.sql 패키지에서 제공
		
		//* JDBC를 이용한 어플리케이션 만들때 필요한 것*
		//1. Java의 JDBC 관련 인터페이스
		//2. DBMS (Oracle)
		//3. Oracle에서 Java 와 연결할때 사용할
		//   JDBC 를 상속받아 구현할 클래스 모음(ojdbc10.jar라이브러리)
		...
		try {
			...
			String ip = "localhost"; //DB 서버 컴퓨터 IP 전달해줘야함
			...
			//3. SQL 작성
			//** JAVA 에서 작성되는 SQL 은 마지막에 ;을 찍으면 안된다!! **
			...
			//executeQuery() : select 문 수행 메서드, resultset 반환
			...
			while(rs.next()) {
				...
				//java.sql.Date의 to.string() 은 yyyy-mm-dd 형식으로 오버라이딩 되어있음
