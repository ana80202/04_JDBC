...
public class MainDAO {
	...
	private PreparedStatement pstmt; // placeholder(위치홀더 : ?)를 포함한 SQL 세팅/수행/걀과반환
	...
	/** 로그인 DAO(아이디,비밀번호 일치하는 회원이 있는지 조회)
	 * @param conn
	 * @param memberId
	 * @param memberPw
	 * @return member
	...
	public Member login(Connection conn, String memberId, String memberPw) throws Exception {
		...
		try {
			...
			//PreparedStatment 객체를 생성하고 SQL를 담아둠
	...
	public int idDuplicationCheck(Connection conn, String memberId) throws Exception {
		...
		try {
			...
			if(rs.next()) {
				result = rs.getInt(1); //<- 첫번쨰 컬럼이라는 뜻
			}
	...
	public int signUp(Connection conn, Member member) throws Exception {
		...
		try {
			...
			result = pstmt.executeUpdate(); //dml=executeUpdate
