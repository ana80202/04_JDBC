...
public class MainView {
	...
	public void mainMenu() {
		...
		do {
			try {
				// 로그인을 했는 지 안했는지를 따져줘야함.
				if(Session.loginMember == null) { //로그인 안했을때
					...
					switch(input) {
					case 1: memberView.memberView(); break;
					case 2 ://MemberService(); break;
	...
	public void SignUp() {
		...
		//아이디, 비밀번호, 비밀번호 확인, 이름, 성별(M/F)를 저장할 변수 선언
		...
		try {
			...
			while(true) {
				...
				if(result == 0) {
					System.out.println("\n====사용 가능한 아이디입니다====\n");
					break; //중복이 아니라 사용가능한 아이디이기떄문에 멈춰줘야한다
				}else {
					System.out.println("\n***이미 사용중인 아이디입니다***\n");
				}
			}
