...
public class TestService {
	...
	public int insert(TestVO vo1) throws SQLException{
		...
		//DAO 메서드 호출하여 수행 수 결과 반환받기
		//-> service에서 생성한 connection 객체를 반드기 같이 전달!
	...
	public int insert(TestVO vo1, TestVO vo2, TestVO vo3) throws Exception {
		...
		int res1 = dao.insert(conn, vo1); // 삽입 성공 하면 1 / 아니면 0
		int res2 = dao.insert(conn, vo2); // 삽입 성공 하면 1 / 아니면 0
		int res3 = dao.insert(conn, vo3); // 삽입 성공 하면 1 / 아니면 0
	...
	public int update(TestVO vo) throws Exception {
		...
		int result = dao.update(conn,vo); //conn 통해서 만들기떄문에 conn반드시 보내줘야함
